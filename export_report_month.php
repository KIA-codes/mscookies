...
/**
 * export_report_month.php
 * Export THIS MONTH's sales report
 */
...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
    try {
        ...
        $details = json_encode([
            ...
            'file_name' => 'sales_report_current_month.pdf',
            'report_type' => "This Month's Sales Report ({$currentMonth})",
        ...
        $description = "User '{$username}' downloaded This Month's Sales Report (PDF)";
...
$html .= '<h2>SALES REPORT SUMMARY (This Month)</h2>';
...
if ($peakPeriod) {
    $html .= "<p>The highest sales were recorded ".$timePeriodText." on day ".$peakPeriod['period'].", reaching Php".number_format($peakPeriod['total'],2).".</p>";
}

$html .= '<h3>SALES THIS MONTH (Last 10 Orders)</h3><table><tr><th>Date</th><th>Customer</th><th>Product</th><th>Qty</th><th>Amount (Php)</th></tr>';
...
$html .= '<h3>TOP 3 PRODUCTS (This Month)</h3><table><tr><th>Product</th><th>Quantity Sold</th></tr>';
...
$html .= '<h2>EXECUTIVE SUMMARY - THIS MONTH</h2>';
$html .= '<p>This month\'s sales performance shows a total of <b>Php'.number_format($totalSales,2).'</b> across <b>'.number_format($totalOrders).' orders</b>.</p>';
$html .= '<p>The average order value of Php'.number_format($avgSales,2).' reflects monthly customer purchasing patterns.</p>';
if ($topProduct) {
    $html .= '<p><b>'.htmlspecialchars($topProduct['Product_Name']).'</b> is the leading item this month with '.$topProduct['qty'].' units sold.';
    if ($peakPeriod) {
        $html .= ' Peak sales occurred on day '.$peakPeriod['period'].' of the month.';
    }
    $html .= '</p>';
}
$html .= '<p><b>Key Insight:</b> This report focuses exclusively on this month\'s sales activity, providing monthly performance metrics for strategic planning.</p>';
...
$dompdf->stream("sales_report_this_month.pdf", ["Attachment" => true]);
