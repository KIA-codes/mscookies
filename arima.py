...
for i in range(len(test_series)):
    ...
    if len(train) < 3:
        ...
        try:
            model = ARIMA(train, order=(0,1,1))  # You can tune order (p,d,q)
            fit = model.fit()
...
all_months["Difference"] = all_months["Actual_Sales"] - all_months["ARIMA_Forecast"]
all_months["Reached?"] = np.where(all_months["Actual_Sales"] >= all_months["ARIMA_Forecast"], "Yes", "No")
...
def compute_metrics(y_true, y_pred, reached_col=None):
    ...
    n = len(y_true); k = 3  # k=3 for ARIMA (p,d,q)
...
metrics_df = pd.DataFrame({
    ...
    "ARIMA": compute_metrics(all_months["Actual_Sales"], all_months["ARIMA_Forecast"], all_months["Reached?"])
})

# ==== 12-Month Future Forecast ====
arima_model = ARIMA(monthly_sales, order=(0,1,1))  # same order as backtest
arima_fit = arima_model.fit()
future_forecast = arima_fit.forecast(12)
...
print("\n=== 12-Month Future Forecast (ARIMA) ===")
