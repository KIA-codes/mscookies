...
for i in range(len(test_series)):
    ...
    results.append({"DATE": test_month, "Forecasted_Sales": forecast, "Actual_Sales": actual_value})
...
rf.fit(X_time, y_resid)
resid_pred = rf.predict(X_time)
...
metrics_df = pd.DataFrame({
    ...
    "SARIMA + RF": compute_metrics(all_months["Actual_Sales"], all_months["Hybrid_Forecast"], all_months["Reached?"])
})
...
future_resid_pred = rf.predict(future_X)
