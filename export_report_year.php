...
/**
 * export_report_year.php
 * Export THIS YEAR's sales report
 */
...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
    try {
        ...
        $details = json_encode([
            ...
            'file_name' => "sales_report_year_{$currentYear}.pdf",
            'report_type' => "This Year's Sales Report ({$currentYear})",
        ...
        $description = "User '{$username}' downloaded This Year's Sales Report (PDF)";
...
$html .= '<h2>SALES REPORT SUMMARY (This Year)</h2>';
...
$html .= '<h3>SALES THIS YEAR (Last 10 Orders)</h3><table><tr><th>Date</th><th>Customer</th><th>Product</th><th>Qty</th><th>Amount (Php)</th></tr>';
...
$html .= '<h3>TOP 3 PRODUCTS (This Year)</h3><table><tr><th>Product</th><th>Quantity Sold</th></tr>';
...
$html .= '<h2>EXECUTIVE SUMMARY - THIS YEAR</h2>';
$html .= '<p>This year\'s sales performance shows a total of <b>Php'.number_format($totalSales,2).'</b> across <b>'.number_format($totalOrders).' orders</b>.</p>';
$html .= '<p>The average order value of Php'.number_format($avgSales,2).' reflects annual customer purchasing patterns.</p>';
if ($topProduct) {
    $html .= '<p><b>'.htmlspecialchars($topProduct['Product_Name']).'</b> is the leading item this year with '.$topProduct['qty'].' units sold.';
    if ($peakPeriod) {
        $html .= ' Peak sales occurred in '.$peakPeriod['period'].'.';
    }
    $html .= '</p>';
}
$html .= '<p><b>Key Insight:</b> This report focuses exclusively on this year\'s sales activity, providing annual performance metrics for long-term strategic planning.</p>';
...
$dompdf->stream("sales_report_this_year.pdf", ["Attachment" => true]);
