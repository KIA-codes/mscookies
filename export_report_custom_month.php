...
/**
 * export_report_custom_month.php
 * - Exports report for a specific month and year
 * - Uses Python JSON output for hybrid forecasts (robust parsing)
 * - Gets actual sales for the specified month from DB and merges with hybrid forecasts
 * - Builds PDF via Dompdf
 */

// Get the custom month and year from URL parameters
$customMonth = isset($_GET['custom_month']) ? $_GET['custom_month'] : date('m');
$customYear = isset($_GET['custom_year']) ? $_GET['custom_year'] : date('Y');
$customDate = $customYear . '-' . str_pad($customMonth, 2, '0', STR_PAD_LEFT);
...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
    try {
        ...
        $monthName = date('F', mktime(0, 0, 0, $customMonth, 1));
        $details = json_encode([
            'username' => $username,
            'file_type' => 'PDF Report',
            'file_name' => "sales_report_month_{$customYear}_{$customMonth}.pdf",
            'report_type' => "Custom Month Sales Report ({$monthName} {$customYear})",
        ...
        $description = "User '{$username}' downloaded {$monthName} {$customYear} Sales Report (PDF)";
...
$topProductQuery = $conn->query("
    ...
    WHERE DATE_FORMAT(s.Sales_Date, '%Y-%m') = '$customDate'
...
// Peak sales day for the month
$peakDayQuery = $conn->query("
    SELECT DATE_FORMAT(Sales_Date, '%Y-%m-%d') as day, SUM(Subtotal) as total
    FROM sales
    WHERE DATE_FORMAT(Sales_Date, '%Y-%m') = '$customDate'
    GROUP BY day
    ORDER BY total DESC
    LIMIT 1
");
$peakDay = $peakDayQuery->fetch_assoc();

// Sales for the month
$monthlySalesQuery = $conn->query("
    SELECT DATE_FORMAT(Sales_Date, '%b %d, %Y') as date, 
           Customer_Name, 
           Subtotal, 
           p.Product_Name, 
           s.Quantity
...
");
$monthlySales = $monthlySalesQuery->fetch_all(MYSQLI_ASSOC);

// Top 3 products for the month
$topMonthlyProductsQuery = $conn->query("
    SELECT p.Product_Name, SUM(s.Quantity) as qty
    FROM sales s
    JOIN product p ON s.Product_ID = p.Product_ID
    WHERE DATE_FORMAT(s.Sales_Date, '%Y-%m') = '$customDate'
    ...
    LIMIT 3
");
$topMonthlyProducts = $topMonthlyProductsQuery->fetch_all(MYSQLI_ASSOC);
...
/* --------------------------
   Get actual sales for the specified month from DB and merge with hybridForecast
   -------------------------- */
$actualSales = [];
...
// Current date and month name
$reportDate = date("F d, Y");
$monthName = date("F Y", strtotime($customDate . '-01'));
...
$html .= '<h2>SALES REPORT SUMMARY - '.$monthName.'</h2>';
$html .= '<p>The system has recorded a total of Php'.number_format($totalSales,2).' in sales across '.number_format($totalOrders).' orders for '.$monthName.'.</p>';
$html .= '<p>On average, each order contributes Php'.number_format($avgSales,2).' in sales.</p>';
if ($topProduct) {
    $html .= "<p>The top-selling product for ".$monthName." is ".htmlspecialchars($topProduct['Product_Name'])." with ".$topProduct['qty']." units sold.</p>";
}
if ($peakDay) {
    $html .= "<p>The highest sales were recorded on ".date('F d, Y', strtotime($peakDay['day'])).", reaching Php".number_format($peakDay['total'],2).".</p>";
}
$html .= "<p>Sales trends for ".$monthName." are shown below, including the top 3 products for the month.</p>";

// Monthly sales
$html .= '<h3>SALES FOR '.$monthName.'</h3><table><tr><th>Date</th><th>Customer</th><th>Product</th><th>Qty</th><th>Amount (Php)</th></tr>';
...
$html .= '</table>';
$html .= '<div class="summary">Insight: This table shows all sales for '.$monthName.', useful for monitoring monthly performance and customer demand patterns.</div>';

// Top monthly products
$html .= '<h3>TOP 3 PRODUCTS FOR '.$monthName.'</h3><table><tr><th>Product</th><th>Quantity Sold</th></tr>';
foreach ($topMonthlyProducts as $prod) {
    $html .= "<tr><td>".htmlspecialchars($prod['Product_Name'])."</td><td>{$prod['qty']} pcs</td></tr>";
}
$html .= '</table>';
$html .= '<div class="summary">Insight: These products were the most frequently purchased during '.$monthName.', highlighting monthly customer preferences.</div>';
...
// Actual vs Hybrid table for the month
$html .= '<h3>ACTUAL vs HYBRID FORECAST FOR '.$monthName.'</h3>';
if (!empty($actualVsHybrid)) {
    ...
    $html .= '</table>';
    $html .= '<div class="summary">Insight: This comparison shows how close the forecast is to actual results for '.$monthName.'. Smaller errors mean better alignment with real sales.</div>';
} else {
    $html .= '<p>No hybrid forecast data available for '.$monthName.'.</p>';
}
...
$html .= '<div style="page-break-before:always;"></div>';
$html .= '<h2>EXECUTIVE SUMMARY - '.$monthName.'</h2>';
$html .= '<p>The analysis for '.$monthName.' confirms that MSC Cookies maintained steady sales performance with a total of Php'.number_format($totalSales,2).' across '.number_format($totalOrders).' orders. The average order value of Php'.number_format($avgSales,2).' indicates stable customer purchasing patterns for the month.</p>';
...
if ($topProduct) {
    $html .= '<p>Product-level insights reveal that <b>'.htmlspecialchars($topProduct['Product_Name']).'</b> was the leading item for '.$monthName.', while the peak sales day of '.($peakDay ? date('F d, Y', strtotime($peakDay['day'])) : 'N/A').' highlights daily opportunities.</p>';
}
$html .= '<p><b>In summary:</b> MSC Cookies demonstrated strong market performance in '.$monthName.', supported by accurate forecasts, high-demand products, and daily growth opportunities.</p>';
...
$dompdf->render();
$dompdf->stream("sales_report_month_".$customDate.".pdf", ["Attachment" => true]);
