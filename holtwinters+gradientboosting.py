...
gb.fit(X_time, y_resid)
resid_pred = gb.predict(X_time)
...
def compute_metrics(y_true, y_pred, reached_col=None):
    ...
    # Mask out zeros for MAPE calculation
    mask = y_true > 0
    if mask.sum() > 0:
...
metrics_df = pd.DataFrame({
    ...
    "HW + GradientBoost": compute_metrics(all_months["Actual_Sales"], all_months["Hybrid_Forecast"], all_months["Reached?"])
})

# Round values for readability
metrics_df.iloc[:, 1:] = metrics_df.iloc[:, 1:].applymap(lambda x: round(x, 2) if pd.notnull(x) else x)
...
future_resid_pred = gb.predict(future_X)
future_hybrid = future_forecast.values + future_resid_pred

future_df = pd.DataFrame({
    "DATE": future_dates,
    "HW_Forecast": np.round(future_forecast.values, 2),
    "Hybrid_Forecast": np.round(future_hybrid, 2)
})
