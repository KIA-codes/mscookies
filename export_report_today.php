...
/**
 * export_report_today.php
 * Export TODAY's sales report
 */
...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
    try {
        ...
        $details = json_encode([
            ...
            'file_name' => 'sales_report_today.pdf',
            'report_type' => "Today's Sales Report",
        ...
        $description = "User '{$username}' downloaded Today's Sales Report (PDF)";
...
$recentSalesQuery = $conn->query("
    SELECT DATE_FORMAT(s.Sales_Date, '%b %d, %Y %H:%i') as date, s.Customer_Name, s.Subtotal, p.Product_Name, s.Quantity
    FROM sales s
    JOIN product p ON s.Product_ID = p.Product_ID
    $whereClause
...
$html .= '<h2>SALES REPORT SUMMARY (Today)</h2>';
...
$html .= "<p>Sales trends ".$timePeriodText." are shown below, including the top 3 products.</p>";

// Recent sales
$html .= '<h3>SALES TODAY (Last 10 Orders)</h3><table><tr><th>Date/Time</th><th>Customer</th><th>Product</th><th>Qty</th><th>Amount (Php)</th></tr>';
...
$html .= '</table>';
$html .= '<div class="summary">Insight: This table shows today\'s orders, useful for monitoring real-time performance.</div>';

// Top recent products
$html .= '<h3>TOP 3 PRODUCTS (Today)</h3><table><tr><th>Product</th><th>Quantity Sold</th></tr>';
foreach ($topRecentProducts as $prod) {
    $html .= "<tr><td>".htmlspecialchars($prod['Product_Name'])."</td><td>{$prod['qty']} pcs</td></tr>";
}
$html .= '</table>';
$html .= '<div class="summary">Insight: These products are the most popular today.</div>';

// Page break
// Executive summary
$html .= '<div style="page-break-before:always;"></div>';
$html .= '<h2>EXECUTIVE SUMMARY - TODAY</h2>';
$html .= '<p>Today\'s sales performance shows a total of <b>Php'.number_format($totalSales,2).'</b> across <b>'.number_format($totalOrders).' orders</b>.</p>';
$html .= '<p>The average order value of Php'.number_format($avgSales,2).' indicates customer purchasing patterns for today.</p>';
if ($topProduct) {
    $html .= '<p>Product-level insights reveal that <b>'.htmlspecialchars($topProduct['Product_Name']).'</b> is the leading item today with '.$topProduct['qty'].' units sold.';
    if ($peakPeriod) {
        $html .= ' Peak sales occurred at '.$peakPeriod['period'].'.';
    }
    $html .= '</p>';
}
$html .= '<p><b>Key Insight:</b> This report focuses exclusively on today\'s sales activity, providing real-time performance metrics for immediate business decisions.</p>';
...
$dompdf->stream("sales_report_today.pdf", ["Attachment" => true]);
