...
/**
 * export_report_custom_year.php
 * - Exports report for a specific year
 * - Uses Python JSON output for hybrid forecasts (robust parsing)
 * - Gets actual sales for the specified year from DB and merges with hybrid forecasts
 * - Builds PDF via Dompdf
 */

// Get the custom year from URL parameter
...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
    try {
        ...
        $details = json_encode([
            ...
            'file_name' => "sales_report_year_{$customYear}.pdf",
            'report_type' => "Custom Year Sales Report ({$customYear})",
        ...
        $description = "User '{$username}' downloaded {$customYear} Sales Report (PDF)";
...
// Sales for the year (monthly breakdown)
$yearlySalesQuery = $conn->query("
    SELECT DATE_FORMAT(Sales_Date, '%b %Y') as date, 
           Customer_Name, 
           Subtotal, 
           p.Product_Name, 
           s.Quantity
    ...
    LIMIT 20
");
$yearlySales = $yearlySalesQuery->fetch_all(MYSQLI_ASSOC);

// Top 3 products for the year
$topYearlyProductsQuery = $conn->query("
    SELECT p.Product_Name, SUM(s.Quantity) as qty
    FROM sales s
    JOIN product p ON s.Product_ID = p.Product_ID
    WHERE YEAR(s.Sales_Date) = '$customYear'
    ...
    LIMIT 3
");
$topYearlyProducts = $topYearlyProductsQuery->fetch_all(MYSQLI_ASSOC);
...
/* --------------------------
   Get actual sales for the specified year from DB and merge with hybridForecast
   -------------------------- */
$actualSales = [];
...
$html .= '<h2>SALES REPORT SUMMARY - YEAR '.$customYear.'</h2>';
$html .= '<p>The system has recorded a total of Php'.number_format($totalSales,2).' in sales across '.number_format($totalOrders).' orders for the year '.$customYear.'.</p>';
$html .= '<p>On average, each order contributes Php'.number_format($avgSales,2).' in sales.</p>';
if ($topProduct) {
    $html .= "<p>The top-selling product for ".$customYear." is ".htmlspecialchars($topProduct['Product_Name'])." with ".$topProduct['qty']." units sold.</p>";
}
if ($peakMonth) {
    $html .= "<p>The highest sales were recorded in ".$peakMonth['month'].", reaching Php".number_format($peakMonth['total'],2).".</p>";
}
$html .= "<p>Sales trends for ".$customYear." are shown below, including the top 3 products for the year.</p>";

// Yearly sales
$html .= '<h3>SALES FOR YEAR '.$customYear.' (Latest 20 Orders)</h3><table><tr><th>Date</th><th>Customer</th><th>Product</th><th>Qty</th><th>Amount (Php)</th></tr>';
...
$html .= '</table>';
$html .= '<div class="summary">Insight: This table shows sales for the year '.$customYear.', useful for monitoring annual performance and customer demand patterns.</div>';

// Top yearly products
$html .= '<h3>TOP 3 PRODUCTS FOR YEAR '.$customYear.'</h3><table><tr><th>Product</th><th>Quantity Sold</th></tr>';
foreach ($topYearlyProducts as $prod) {
    $html .= "<tr><td>".htmlspecialchars($prod['Product_Name'])."</td><td>{$prod['qty']} pcs</td></tr>";
}
$html .= '</table>';
$html .= '<div class="summary">Insight: These products were the most frequently purchased during '.$customYear.', highlighting annual customer preferences.</div>';
...
// Actual vs Hybrid table for the year
$html .= '<h3>ACTUAL vs HYBRID FORECAST FOR YEAR '.$customYear.'</h3>';
if (!empty($actualVsHybrid)) {
    ...
    $html .= '</table>';
    $html .= '<div class="summary">Insight: This comparison shows how close the forecast is to actual results for '.$customYear.'. Smaller errors mean better alignment with real sales.</div>';
} else {
    $html .= '<p>No hybrid forecast data available for the year '.$customYear.'.</p>';
}
...
$html .= '<div style="page-break-before:always;"></div>';
$html .= '<h2>EXECUTIVE SUMMARY - YEAR '.$customYear.'</h2>';
$html .= '<p>The analysis for '.$customYear.' confirms that MSC Cookies maintained steady sales performance with a total of Php'.number_format($totalSales,2).' across '.number_format($totalOrders).' orders. The average order value of Php'.number_format($avgSales,2).' indicates stable customer purchasing patterns for the year.</p>';
...
if ($topProduct) {
    $html .= '<p>Product-level insights reveal that <b>'.htmlspecialchars($topProduct['Product_Name']).'</b> was the leading item for '.$customYear.', while the peak sales month of '.($peakMonth['month'] ?? 'N/A').' highlights seasonal opportunities.</p>';
}
$html .= '<p><b>In summary:</b> MSC Cookies demonstrated strong market performance in '.$customYear.', supported by accurate forecasts, high-demand products, and seasonal growth opportunities.</p>';
...
$dompdf->render();
$dompdf->stream("sales_report_year_".$customYear.".pdf", ["Attachment" => true]);
