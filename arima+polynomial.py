# %% ARIMA + Polynomial Regression Hybrid
import pandas as pd
import numpy as np
...
# ==== Backtest range: start from first month ====
...
# ==== Residual modelling with Polynomial Regression (degree=2) ====
...
poly_pipeline.fit(X_time, y_resid)
resid_pred = poly_pipeline.predict(X_time)

all_months["Hybrid_Forecast"] = all_months["ARIMA_Forecast"] + resid_pred

# ==== Reached? & Diff (hybrid only) ====
...
future_hybrid = future_forecast.values + poly_pipeline.predict(future_X)
...
# ==== Output ====
print("\n=== Metrics (ARIMA + Polynomial Regression) ===")
